...
export const NotificationProvider = ({ children, userId }) => {
        ...
        try {
          ...
          if (deviceToken && deviceToken.data) {
            ...
            if (token.startsWith("ExponentPushToken[")) {
              console.warn(
                "⚠️ WARNING: Got Expo token instead of FCM token (Admin)",
              );
              console.warn(
  ...
  const broadcastNotification = useCallback(
    async (appType, title, body, data = {}) => {
      try {
        // Fetch all active tokens for the app type
  ...
  const notifySeller = useCallback(
    async (sellerId, title, body, data = {}) => {
      return sendNotification({
        userId: sellerId,
        appType: "seller",
        title,
